...
export const SignUpSuccess = ({ email, onBack }: SignUpSuccessProps) => {
  return (
    <motion.div key="success" initial={{ scale: 0.98, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="w-full">
      <div className="p-10! bg-bg2/20 border border-green-500/10 rounded-lg! border-white/5!">
        <div className="w-16! h-16! bg-green-500/10 rounded-lg! flex items-center justify-center mb-8! border border-green-500/20">
        ...
        <PremiumButton variant="secondary" onClick={onBack} className="w-full! py-4! rounded-lg!">
          Return to Sign In
        </PremiumButton>
      </div>
    </motion.div>
